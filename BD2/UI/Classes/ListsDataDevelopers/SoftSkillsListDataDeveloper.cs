...
{
    public class SoftSkillsListDataDeveloper : AbstractListDataDeveloper
    {
        public override List<object> LoadData(int pageNumber)
        {
            ...
            try
            {
                ...
                {
                    returnList = service.GetSoftSkillsByPage(pageNumber).Cast<object>().ToList();
                }
            }
            catch(Exception e)
            {
        ...
        public override int GetAmountOfRecords()
        {
            ...
            try
            {
                using(var service = new DictionaryServiceClient())
                {
                    amount = service.GetAmountOfSoftSkills();
                }
            }
            catch(Exception e)
            {
